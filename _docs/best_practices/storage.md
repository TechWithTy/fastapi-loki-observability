...
This guide assumes a provisioned EKS cluster.
    ...
    Checkout the Loki repository and navigate to production/terraform/modules/s3.

    Initialize Terraform terraform init.

    Export the AWS profile and region if not done so:
    bash 
...
bash
    ...
    Apply the Terraform module terraform -var region="$AWS_REGION" -var cluster_name=<EKS cluster> -var oidc_id="$oidc_provider"
